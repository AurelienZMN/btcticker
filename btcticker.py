...
configfile = os.path.join(os.path.dirname(os.path.realpath(__file__)),
                          'config.yaml')
font = ImageFont.truetype(os.path.join(fontdir,
                          'googlefonts/Roboto-Medium.ttf'), 40)
fontHorizontal = ImageFont.truetype(os.path.join(fontdir,
                                    'googlefonts/Roboto-Medium.ttf'),
                                    50)
font_date = ImageFont.truetype(os.path.join(fontdir,
                               'PixelSplitter-Bold.ttf'), 11)
...
def getData():
    ...
    try:
        url = 'https://rest.coinapi.io/v1/exchangerate/BTC/CAD?apikey=' \
            + COINAPI_KEY
    ...
    try:
        url = \
            'https://rest.coinapi.io/v1/ohlcv/BTC/CAD/latest?period_id=7DAY&apikey=' \
            + COINAPI_KEY
...
def updateDisplay(config, pricestack):
    ...
    if config['ticker']['hidden'] == True:
        if config['display']['orientation'] == 0 or config['display'
            ...
            draw.text((0, 10), str(time.strftime('%c')),
        ...
        if config['display']['orientation'] == 90 or config['display'
            ...
            draw.text((85, 5), str(time.strftime('%c')),
                      font=font_date, fill=0)
    ...
    #       This is a hack to deal with the mirroring that goes on in 4Gray Horizontal
        ...
        if config['display']['orientation'] == 0 or config['display'
            ...
            draw.text((110, 95), str('%+d' % round((pricestack[-1]
                      - pricestack[1]) / pricestack[-1] * 100, 2)) + '%'
                      , font=font_date, fill=0)
            draw.text((5, 200), '$' + format(int(round(BTC)), ','),
                      font=font, fill=0)
            draw.text((0, 10), str(time.strftime('%c')),
                      font=font_date, fill=0)
        ...
        if config['display']['orientation'] == 90 or config['display'
            ...
            image = Image.new('L', (epd.height, epd.width), 255)  # 255: clear the image with white
            draw = ImageDraw.Draw(image)
            draw.text((100, 100), '7day : ' + str('%+d'
                      % round((pricestack[-1] - pricestack[1])
                      / pricestack[-1] * 100, 2)) + '%',
                      font=font_date, fill=0)
            draw.text((20, 120), '$' + format(int(round(BTC)), ','),
                      font=fontHorizontal, fill=0)
...
#       Get the configuration from config.yaml
        ...
        config['display']['orientation'] = int(config['display'
                ]['orientation'])
        ...
        while True:
            ...
            if internet():
                ...
                if key2state == False:
                    ...
                    config['display']['orientation'] = (config['display'
                            ]['orientation'] + 90) % 360
                    time.sleep(0.2)
                ...
                if key4state == False:
                    ...
                    updateDisplay(config, pricestack)
                    time.sleep(0.2)
                if time.time() - lastcoinfetch > float(config['ticker'
                        ]['updatefrequency']) or datapulled == False:
